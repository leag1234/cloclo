...
- **GitHub token**: TWO exposure incidents occurred (2026-09-13 and 2026-09-15). BOTH are
  CLOSED. After the 2026-09-15 incident the token was regenerated by the owner, granted
  Contents/Pull-requests/Workflows write access, verified working (API write 201, delete
  200, `git push` succeeded), and re-injected into `secrets.env` and the git remote. The
  token currently in use is the post-2026-09-15 one. No further confirmation is needed for
  either incident: do not stop on a token exposure recorded before 2026-09-17.
