...
class ImageToolBudgetTests(unittest.IsolatedAsyncioTestCase):
    async def test_decision_and_image_share_one_budget(self) -> None:
        for spent in (Decimal("0.01"), Decimal("0.05")):
            model = GatewayModel(
                "http://unused",
                Configuration(
            ...
            with (
                ...
                patch.object(
                    model,
                    "estimate",
                    Mock(return_value=Reservation(100, Decimal("0.05"))),
                ),
                ...
                if spent == Decimal("0.05"):
                    ...
                    self.assertEqual(Decimal(payload["max_cost_eur"]), Decimal("0.04"))
                    self.assertLess(payload["timeout"], 120)
                    self.assertEqual(payload["lang"], "fr")
                    self.assertAlmostEqual(item.cout_eur, 0.011)
